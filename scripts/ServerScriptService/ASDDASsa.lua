...
local function doCommit()
	...
	if r.success then
		...
		local summaryLine = ""
		if r.summary then
			summaryLine = string.format("%d added, %d deleted, %d modified\n\n",
				r.summary.added or 0, r.summary.deleted or 0, r.summary.modified or 0)
		end
		if r.diff and r.diff ~= "" then
			GUI:setDiff(summaryLine .. r.diff)
		else
			GUI:setDiff(summaryLine .. "Commit " .. short .. ": sem mudanças textuais (só metadados/UID)")
		end
...
local function buildPreviewLists(targetFiles)
	local current = scanScripts()
	local added, deleted, modified = {}, {}, {}
	for path in pairs(targetFiles) do
		if not current[path] then table.insert(added, path) end
	end
	for path, info in pairs(current) do
		if not targetFiles[path] then table.insert(deleted, path) end
	end
	for path, info in pairs(current) do
		if targetFiles[path] and targetFiles[path] ~= info.source then
			table.insert(modified, path)
		end
	end
	table.sort(added); table.sort(deleted); table.sort(modified)
	return added, deleted, modified
end

local function showBranchPreview(name, r, onApply, onCancel)
	local sg = GUI.widget and GUI.widget.Parent
	if not sg then onApply() return end
	local added, deleted, modified = buildPreviewLists(r.files)
	if #added == 0 and #deleted == 0 and #modified == 0 then
		onApply()
		return
	end

	local existing = sg:FindFirstChild("BranchPreviewPopup")
	if existing then existing:Destroy() end

	local popup = Instance.new("Frame")
	popup.Name = "BranchPreviewPopup"
	popup.Size = UDim2.new(0, 380, 0, 320)
	popup.Position = UDim2.new(0.5, -190, 0.5, -160)
	popup.BackgroundColor3 = Color3.fromRGB(25, 25, 32)
	popup.BorderSizePixel = 1; popup.BorderColor3 = Color3.fromRGB(60, 60, 70)
	popup.ZIndex = 350; popup.Parent = sg

	local hd = Instance.new("Frame"); hd.Size = UDim2.new(1, 0, 0, 28)
	hd.BackgroundColor3 = Color3.fromRGB(40, 40, 48); hd.Parent = popup
	local ht = Instance.new("TextLabel"); ht.Size = UDim2.new(1, -16, 1, 0); ht.Position = UDim2.new(0, 8, 0, 0)
	ht.BackgroundTransparency = 1; ht.TextColor3 = Color3.fromRGB(220, 220, 240); ht.Font = Enum.Font.GothamBold
	ht.TextSize = 13; ht.Text = "Preview: switching to " .. name
	ht.TextXAlignment = Enum.TextXAlignment.Left; ht.Parent = hd

	local list = Instance.new("ScrollingFrame")
	list.Size = UDim2.new(1, -8, 1, -70); list.Position = UDim2.new(0, 4, 0, 30)
	list.BackgroundTransparency = 1; list.ScrollBarThickness = 4
	list.CanvasSize = UDim2.new(0, 0, 0, 0); list.Parent = popup
	local ll = Instance.new("UIListLayout"); ll.Padding = UDim.new(0, 1); ll.Parent = list
	...
	local function addLine(text, color)
		...
		l.Text = text; l.Parent = list
		y = y + 17
	end
	for _, p in ipairs(added) do addLine("+ " .. p, Color3.fromRGB(100, 255, 100)) end
	for _, p in ipairs(deleted) do addLine("- " .. p, Color3.fromRGB(255, 100, 100)) end
	for _, p in ipairs(modified) do addLine("~ " .. p, Color3.fromRGB(255, 220, 100)) end
	list.CanvasSize = UDim2.new(0, 0, 0, y + 4)

	local summary = Instance.new("TextLabel")
	summary.Size = UDim2.new(1, -16, 0, 16); summary.Position = UDim2.new(0, 8, 1, -68)
	summary.BackgroundTransparency = 1; summary.TextColor3 = Color3.fromRGB(180, 180, 190)
	summary.Font = Enum.Font.Gotham; summary.TextSize = 10
	summary.Text = string.format("%d added · %d deleted · %d modified", #added, #deleted, #modified)
	summary.TextXAlignment = Enum.TextXAlignment.Left; summary.Parent = popup

	local applyBtn = Instance.new("TextButton")
	applyBtn.Size = UDim2.new(0.5, -6, 0, 28); applyBtn.Position = UDim2.new(0, 4, 1, -32)
	applyBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 80); applyBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	applyBtn.Font = Enum.Font.GothamBold; applyBtn.TextSize = 12; applyBtn.Text = "Apply"
	...
	cancelBtn.Size = UDim2.new(0.5, -6, 0, 28); cancelBtn.Position = UDim2.new(0.5, 2, 1, -32)
	cancelBtn.BackgroundColor3 = Color3.fromRGB(80, 30, 30); cancelBtn.TextColor3 = Color3.fromRGB(255, 130, 130)
	cancelBtn.Font = Enum.Font.GothamBold; cancelBtn.TextSize = 12; cancelBtn.Text = "Cancel"
	...
	if state.dirty then
		...
		if sg then
			...
			skBtn.MouseButton1Click:Connect(function()
				unsavedPopup:Destroy()
				showBranchPreview(name, r, doSafetyCheck, cancelSwitch)
			end)
...
local function showMergeConflictPopup(src, tgt, conflicts)
	local sg = GUI.widget and GUI.widget.Parent
	if not sg then return end
	local existing = sg:FindFirstChild("MergeConflictPopup")
	if existing then existing:Destroy() end

	local resolutions = {}
	for _, c in ipairs(conflicts) do
		resolutions[c.path] = "source"
	end

	local popup = Instance.new("Frame")
	popup.Name = "MergeConflictPopup"
	popup.Size = UDim2.new(0, 460, 0, 420)
	popup.Position = UDim2.new(0.5, -230, 0.5, -210)
	popup.BackgroundColor3 = Color3.fromRGB(25, 25, 32)
	popup.BorderSizePixel = 1; popup.BorderColor3 = Color3.fromRGB(60, 60, 70)
	...
	local ht = Instance.new("TextLabel"); ht.Size = UDim2.new(1, -16, 1, 0); ht.Position = UDim2.new(0, 8, 0, 0)
	ht.BackgroundTransparency = 1; ht.TextColor3 = Color3.fromRGB(220, 220, 240); ht.Font = Enum.Font.GothamBold
	ht.TextSize = 13; ht.Text = "Conflicts: " .. src .. " → " .. tgt
	ht.TextXAlignment = Enum.TextXAlignment.Left; ht.Parent = hd
	...
	for _, c in ipairs(conflicts) do
		...
		pathL.Size = UDim2.new(1, -8, 0, 16); pathL.Position = UDim2.new(0, 4, 0, 2)
		pathL.BackgroundTransparency = 1; pathL.TextColor3 = Color3.fromRGB(220, 220, 230)
		pathL.Font = Enum.Font.Code; pathL.TextSize = 11
		pathL.TextXAlignment = Enum.TextXAlignment.Left; pathL.Text = c.path; pathL.Parent = row

		local srcBtn = Instance.new("TextButton")
		srcBtn.Size = UDim2.new(0.5, -8, 0, 22); srcBtn.Position = UDim2.new(0, 4, 0, 22)
		srcBtn.Font = Enum.Font.GothamBold; srcBtn.TextSize = 10
		srcBtn.Text = "✓ Use " .. src; srcBtn.Parent = row

		local tgtBtn = Instance.new("TextButton")
		tgtBtn.Size = UDim2.new(0.5, -8, 0, 22); tgtBtn.Position = UDim2.new(0.5, 4, 0, 22)
		tgtBtn.Font = Enum.Font.GothamBold; tgtBtn.TextSize = 10
		tgtBtn.Text = "Keep " .. tgt; tgtBtn.Parent = row

		local previewL = Instance.new("TextLabel")
		previewL.Size = UDim2.new(1, -8, 0, 28); previewL.Position = UDim2.new(0, 4, 0, 46)
		previewL.BackgroundTransparency = 1; previewL.TextColor3 = Color3.fromRGB(150, 150, 160)
		previewL.Font = Enum.Font.Code; previewL.TextSize = 9; previewL.TextWrapped = true
		previewL.TextXAlignment = Enum.TextXAlignment.Left
		previewL.Text = src .. ": " .. c.source:sub(1, 40):gsub("\n", " ") .. " | " .. tgt .. ": " .. c.target:sub(1, 40):gsub("\n", " ")
		previewL.Parent = row

		local function refreshChoice()
			local chosen = resolutions[c.path]
			srcBtn.BackgroundColor3 = chosen == "source" and Color3.fromRGB(0, 140, 80) or Color3.fromRGB(60, 60, 68)
			srcBtn.TextColor3 = chosen == "source" and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(200, 200, 210)
			tgtBtn.BackgroundColor3 = chosen == "target" and Color3.fromRGB(0, 140, 80) or Color3.fromRGB(60, 60, 68)
			tgtBtn.TextColor3 = chosen == "target" and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(200, 200, 210)
		end
		srcBtn.MouseButton1Click:Connect(function() resolutions[c.path] = "source"; refreshChoice() end)
		tgtBtn.MouseButton1Click:Connect(function() resolutions[c.path] = "target"; refreshChoice() end)
		refreshChoice()
	...
	confirmBtn.Size = UDim2.new(1, -8, 0, 30); confirmBtn.Position = UDim2.new(0, 4, 1, -34)
	confirmBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 212); confirmBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	confirmBtn.Font = Enum.Font.GothamBold; confirmBtn.TextSize = 12; confirmBtn.Text = "Apply resolution"
	confirmBtn.Parent = popup
	confirmBtn.MouseButton1Click:Connect(function()
		local r2 = RPC:send("merge_resolved", {place = state.place, source = src, target = tgt, resolutions = resolutions})
		if r2.success then
			log("✓ Merge resolved: " .. src .. " → " .. tgt)
			popup:Destroy()
		else
			log("✗ Merge resolve: " .. (r2.error or "failed"))
		end
	end)
end
...
local function handleCommand(cmd)
	...
	elseif cmd:match("^merge ") then
		...
		if r.success then
			log("✓ Merged " .. src .. " → " .. tgt)
		elseif r.conflicts and #r.conflicts > 0 then
			log("⚠ " .. #r.conflicts .. " conflito(s) — abrindo resolução")
			showMergeConflictPopup(src, tgt, r.conflicts)
		else
			log("✗ Merge: " .. (r.error or "failed"))
		end
